...
//#define ADDRESS 0x3D     // Alternative address

class HVGDISP : public Print
{
  private:
    // Setup I2C comms to display
    void initI2C( void );

    // Send initialisation sequence to display
   // uint8_t	initDisplay(uint8_t pin);
  ...
  public:
	...
	//uint8_t disp_buffer[sizeof(RIP_MSG)]; //

    // Void constructor
    HVGDISP(void) { };

    // Constructor
    HVGDISP(uint8_t resetPin) {
      //uint8_t result = 0;

      // Initialise I2C to display
     	//initI2C();

      // Send initialisation sequence to display over I2C
     	//result = initDisplay(resetPin);
    };
