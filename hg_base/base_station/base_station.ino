 ...
 * - hvg_disp written by Rowan Gifford for http://harvestgeek.com
...
HVGCOMMS hvg_comms(ETH_SS, GUID_INIT_ADDR, GUID_REGED_ADDR, GUID_START_ADDR);
// LCD library - RG 25/06/2014
HVGDISP hvg_disp(LCD_RST);
// Platform MAC address
byte mac[] = { 0xD4, 0xF3, 0x8F, 0xC9, 0xDE, 0xB5 };
...
unsigned char get_string_to_display(char id, char* buffer)
{
  if(display_sm == DISP_ETH_PB)
    strcpy_P(buffer, PSTR("Ethernet problem"));
  else if(display_sm == DISP_DHCP_PB)
    strcpy_P(buffer, PSTR("DHCP problem"));  
  else if(display_sm == DISP_INIT)
    strcpy_P(buffer, PSTR("Initializing..."));  
  else if(display_sm == DISP_INIT_DONE)
    strcpy_P(buffer, PSTR("Initialization done!"));  
  else if(display_sm == DISP_NO_CON)
    strcpy_P(buffer, PSTR("No internet"));      
  else if(display_sm == DISP_PB_COMS)
    strcpy_P(buffer, PSTR("W5100 problem"));      
  else if(display_sm == DISP_NREG)
    strcpy_P(buffer, PSTR("Register your device"));  
  else if(display_sm == DISP_COLLECT)
    strcpy_P(buffer, PSTR("Harvestgeek"));  
  else if(display_sm == DISP_NRF_PB)
    strcpy_P(buffer, PSTR("NRF24 problem"));  
...
void draw(void) 
{
  ...
  if(display_sm == DISP_NREG)
  {
    ...
    hvg_disp.print((char*)hvg_comms.hvg_comms_buffer);
    hvg_comms.convert_hex_to_hexstring(hvg_comms.guid+(GUID_LGTH/2), (char*)hvg_comms.hvg_comms_buffer, GUID_LGTH/2);
 ...
 if((EEPROM.read(INIT_ADDR) != INIT_UID) && (EEPROM.read(INIT_ADDR2) != INIT_UID))
  {
    ...
    for(i = 0; i < 6; i++) {
      EEPROM.write(MAC1_ADDR+i, mac[i]);
    }
  }
  ...
  for(i = 0; i < 6; i++)
    mac[i] = EEPROM.read(MAC1_ADDR+i);
  
  /** Initialize communications **/
  i = 0;
  hvg_comms.init(mac);
  return_hvg_comms = hvg_comms.init(mac);    
  ...
  {      
    ...
    if(return_hvg_comms == RETURN_NREGISTERED)
...
void loop(void) 
{
  ...
  {
    send_survey_packet();
    hvg_comms.send_report(mac, mess_log);
  }
  ...
  // Added RG 25/06/2014
  /** If the user pressed the button for several seconds, ask an ID **/
  if((button_counter == 4) && ((state_machine & PID_INIT) != PID_INIT))
  {
      // Request an ID    
      temp_token = random(0,255);
      nrf_packet.message_type = IDREQ_TYPE;
      nrf_packet.platform_id = temp_token;
  
      for(i = 0; (i < 3) && (state_machine != PID_INIT); i++)
      {  
        while(send_nrf_packet(3) == false);
        temp_sec_counter = 0;
        
        // Wait for reply during 10 secs
        while(temp_sec_counter != 11)
        {
          if(nrf24.waitAvailableTimeout(500))
          {
            if(nrf24.recv((uint8_t*)&nrf_packet, &plen))
            {
              if(Serial)Serial.println("Packet received");
              ...
              if((temp_crc == nrf_packet.crc) && (nrf_packet.platform_id == NO_ID_PID) && (nrf_packet.message_type == IDREP_TYPE) && (nrf_packet.data[0] == temp_token))
              {
                if(Serial){Serial.print("Got new PID: ");Serial.println(nrf_packet.data[1],DEC);}
                platform_id = nrf_packet.data[1];
                EEPROM.write(PID_ADDR, platform_id);
                EEPROM.write(INIT_ADDR2, INIT_UID);
                EEPROM.write(INIT_ADDR, INIT_UID);
                state_machine |= PID_INIT;
              }
            }
          }
        }
      ...
      if((state_machine & PID_INIT) != PID_INIT)
      {
          if(Serial)Serial.println("No answer received");
      }
  }
